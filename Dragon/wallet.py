...
class BulkWalletChecker:
    ...
    def fetchWalletDataOld(self, wallets, threads, skipWallets, useProxies):
    ...
    def fetchWalletData(self, wallets, threads, skipWallets, useProxies):
        ...
        result_array = []
        for wallet, data in result_dict.items():
            wallet_data = {"wallet": wallet}
            wallet_data.update(data)
            result_array.append(wallet_data)

        print(f"[🐲] Processed data for {len(result_array)} wallets.")
        return result_array
