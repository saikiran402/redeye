...
class TopTraders:
    ...
    def topTraderDataOld(self, contractAddresses, threads, useProxies):
    ...
    def topTraderData(self, contractAddresses, threads, useProxies):
        ...
        # Prepare repeated addresses
        repeatedAddresses = [address for address, count in self.addressFrequency.items() if count > 1]
        
        # Combine all the data into a JSON object
        result = {
            "allTopTraders": self.allData,
            "repeatedTopTraders": repeatedAddresses,
            "totalTraders": self.totalTraders,
            "uniqueAddresses": list(self.allAddresses)
        }
        
        print(f"[🐲] Processed {self.totalTraders} top traders for {len(contractAddresses)} tokens.")
        print(f"[🐲] Found {len(self.allAddresses)} unique top trader addresses.")
        print(f"[🐲] Found {len(repeatedAddresses)} repeated trader addresses.")
